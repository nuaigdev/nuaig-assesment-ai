...
import { useBreadcrumbLabels } from "./breadcrumb-label";
...
function buildCrumbs(pathname: string, names: ReadonlyMap<string, string>): Crumb[] {
  ...
  return segments.map((segment, index) => {
    ...
    const label = LABELS[segment] ?? names.get(segment) ?? "Details";
    const linkable =
...
export function Breadcrumbs() {
  const crumbs = buildCrumbs(usePathname(), useBreadcrumbLabels());
        ...
        {crumbs.map((crumb, index) => {
          ...
          return (
            <li key={crumb.href} className="flex min-w-0 items-center gap-1.5">
              ...
              {isLast ? (
                <span aria-current="page" className="max-w-72 truncate font-medium text-fg">
                  {crumb.label}
                </span>
              ) : crumb.linkable ? (
                <Link href={crumb.href} className="max-w-56 truncate rounded-sm text-fg-muted hover:text-fg">
                  {crumb.label}
                </Link>
              ) : (
                <span className="max-w-56 truncate text-fg-muted">{crumb.label}</span>
              )}
            </li>
          );
        })}
