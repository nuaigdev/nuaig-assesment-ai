...
**`spec.md` is the source of truth**, except for the auth deviation described under "Staff auth" below. Read §4–§6 (architecture, services, hard constraints) before writing code. §14 is the work queue: eight phases (0–7), each with scope and exit criteria, built in order. Phases 0 (foundation, SSO, schema, shell), 1 (clients, team, scheduling, magic links, join welcome/consent, audit log) and 2 (LiveKit call: device check, Teams-style stage, listen-only staff, presence, End interview) are built. Phase 3 (agent worker bridging LiveKit to ElevenLabs Speech Engine, scripted replies, live transcript over Realtime) is built in `worker/` and the app, but needs an ElevenLabs key and a worker host before it can run end to end. `/templates` and `/knowledge` are placeholders, and so is the call's Steer tab until Phase 6.
...
- **Calls (LiveKit):**
  - **Tokens:** minted only in `src/lib/livekit/server.ts`. The interviewee's identity is always `interviewee` (the Phase 3 worker pins to it), the agent's is `agent`, and staff are `staff-<userId>`. Roles travel in token `attributes.role`. Staff tokens can't publish anything, and no token can update its own metadata.
  - **Presence:** routes (`/api/join/[token]/presence`, `/api/interviews/[id]/presence`) record a join only after `isInRoom` confirms it with LiveKit, and handle a leave by removing the participant first. They write through the service-role functions `record_participant_presence` and `finalize_interview` (not callable by `authenticated`). An interview goes `live` when the interviewee actually joins.
  - **Teardown:** every way a call ends MUST go through `teardownInterview()` in `src/lib/interviews/call.ts`, the §13.1 checklist that later phases fill in. It finalizes the database, then closes the room, which disconnects everyone.
  - **Client:** `useCall` (`src/components/call/use-call.ts`) must be joined from a user tap, because it calls `room.startAudio()` before any await for iOS. The staff room lives in the `(call)` route group, a dark full-screen surface outside the console layout. `CallStage` renders inside LiveKit's `RoomContext`.
- **Agent worker (`worker/`):**
  - **Package:** a separate npm package with its own lockfile, deployed to an always-on host (Dockerfile). Excluded from the app's tsconfig, ESLint and Vercel upload. Commands: `npm run dev|build|start|typecheck` and `npm run setup:engine` (creates or updates the ElevenLabs Speech Engine) inside `worker/`.
  - **Dispatch:** when the interviewee's join is confirmed, the app dispatches the LiveKit agent `nuaig-interviewer` (`dispatchAgent`). The job joins as identity `agent`, subscribes only to `interviewee`, streams 16 kHz PCM to Speech Engine, and publishes its 24 kHz voice.
  - **Brain:** Speech Engine calls the worker's brain WebSocket (`/brain`, same process as `/healthz`) for each finished turn, and the brain calls the app's `POST /api/agent/turn`.
  - **Worker ⇄ app:** the worker never touches the database. It talks only to `/api/agent/{session,turn,events}` with `Authorization: Bearer AGENT_WORKER_SECRET`.
  - **Brain logic:** lives in the app (`src/lib/agent/`). Phase 3 uses the scripted replies in `script.ts`; Phase 4 swaps in Claude behind the same route.
- **Transcript:** the service-role functions `append_transcript_entry` and `update_transcript_entry` write every line, with `seq` allocated under a lock on the interview row. `/api/agent/turn` writes the agent's line as interim when it's produced, and the worker's `agent_response` event finalizes it (`agent_correction` rewrites it after an interruption). Staff follow along through `LiveTranscript`: Supabase Realtime `postgres_changes`, authorized with a token from `/api/interviews/[id]/realtime-token`, so RLS applies.
